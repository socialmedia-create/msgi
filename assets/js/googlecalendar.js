document.addEventListener("DOMContentLoaded", function () {
  ...
  // Listen for when the News tab becomes visible
  document.getElementById('news-tab').addEventListener('shown.bs.tab', function () {
    if (calendarEl && !calendarInitialized) {
      calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function (info) {
          info.jsEvent.preventDefault(); // Prevent Google redirect
        }
      });

      console.log("Rendering calendar on News tab");
      calendar.render();
      ...
      calendarInitialized = true; // Ensure it renders only once
    } else if (calendar) {
      // If already initialized, just update size on tab switch
      calendar.updateSize();
    }
  });
